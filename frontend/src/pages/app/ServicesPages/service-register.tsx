...
import { api } from "@/lib/axios"; // Certifique-se de que este caminho está correto.
import React, { useState } from "react";
...
export function ServiceRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        name: formData.serviceType,
        price: parseFloat(formData.price.replace(/[^\d,.-]/g, "")), // Remove símbolos
        description: formData.description,
        date: Date.now(), // Data atual
        materials: formData.materials,
      };

      const response = await api.post("/services", payload);
      setSuccessMessage(response.data.message || "Serviço cadastrado com sucesso!");
      ...
      setErrorMessage(
        error.response?.data?.error || "Erro ao cadastrar o serviço. Tente novamente mais tarde."
      );
    } finally {
      setLoading(false);
    }
